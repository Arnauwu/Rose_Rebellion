...
bool Minairon::Update(float dt)
{
	...
	if (isdead && anims.GetCurrentName() != "dead")
	{
		...
		Engine::GetInstance().audio->PlayFx(morirMinairon);
	...
	if (isWalking && !wasWalking) {
		Engine::GetInstance().audio->PlayFx(caminarMinairon, 99);
	}

	else if (!isWalking && wasWalking) {
		Engine::GetInstance().audio->StopFx(caminarMinairon);
	}
...
void Minairon::PerformPathfinding()
{
	//Reset path
	pathfinding->ResetPath(GetTilePos());
...
void Minairon::GetPhysicsValues() {
...
void Minairon::OnCollision(PhysBody* physA, PhysBody* physB, b2ShapeId shapeA, b2ShapeId shapeB)
{
...
void Minairon::OnCollisionEnd(PhysBody* physA, PhysBody* physB, b2ShapeId shapeA, b2ShapeId shapeB)
{
	switch (physB->ctype)
	{
	default:
