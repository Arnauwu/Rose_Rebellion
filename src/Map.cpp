...
Vector2D Map::GetPlayerSpawnPoint(const std::string& fromRoom)
{
	...
	{
		printf("fromroom funcion : %s",fromRoom.c_str());
		printf("fromroom spawnpoint : %s", spawnPoint->fromRoom.c_str());
		printf("position spawnX(3800) : %d// y(912) : %d",spawnPoint->position.getX(), spawnPoint->position.getY());
		if (spawnPoint->fromRoom == fromRoom)
		{
			printf("SpawnPoint : %d",spawnPoint->position);
