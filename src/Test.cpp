...
void TestEnemy::Move() {
	...
	else
	{
		...
		//Jump
		if (nextTile.getY() < tilePos.getY() && onGround)
		{
			Engine::GetInstance().physics->ApplyLinearImpulseToCenter(pbody, 0, -0.75f);
			if (playerTileDist > 5)
			{
				Engine::GetInstance().physics->ApplyLinearImpulseToCenter(pbody, 0, -1.5f);
			}

			onGround = false; //Fail-Save
		}
	}
	return;
}
